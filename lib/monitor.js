...
module.exports = internals.Monitor = class {
    ...
    start() {
        ...
        for (const reporterName in this.settings.reporters) {
            ...
            const pumpified = Pumpify.obj(streamObjs);

            this._toDestroy.push(() => pumpified.forceDestroy());

            this._reporters.set(reporterName, pumpified);
            ...
            const rep = this._reporters.get(reporterName);
            rep.on('error', handler);
            this._toDestroy.push(() => rep.off('error', handler));
        }
